...
class MenuController1 {
  Future<List<MenuSection>> fetchMenuSections({required int restaurantId}) async {
    final response = await supabase.from('menu_section').select().eq('restaurant_id', restaurantId);
    ...
    return (response as List<dynamic>).map((data) => MenuSection.fromMap(data as Map<String, dynamic>)).toList();
  }
  ...
  Future<List<MenuItem>> fetchMenuItems({required int sectionId}) async {
    ...
    try {
      final response = await supabase.from('menu_items').select().eq('section_id', sectionId);
  ...
  fetchMenuItem({required int itemId}) {
    final response = supabase.from('menu_items').select().eq('id', itemId);
  }
}
